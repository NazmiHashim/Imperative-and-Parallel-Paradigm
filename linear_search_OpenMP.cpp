...
// Linear search function with additional features
int LinearSearch(int* arr, int size, int key, int* comparisons)
{
    ...
    *comparisons = 0;
    ...
    printf("\tLinear Search in OpenMP\n");
    printf("==================================================\n");
...
#pragma omp critical
                {
                    index = i;
                    printf("Thread %d found the element at index %d\n", thread_id, index);
                }
            }
        }
    }
...
int main()
{
    ...
    int size = sizeof(arr) / sizeof(arr[0]); //Variable to get the number of element in array
    int key = 119;     // Variable that serve as the key value in linear search
    int comparisons;  // Variable to store the comparisons count

    // Calling LinearSearch function
    int index = LinearSearch(arr, size, key, &comparisons);

    // Printing result and comparisons count
    if (index != -1) {
    ...
    // Print the total comparisons made during the search
    printf("Total comparisons made to find key value: %d\n", comparisons);
