...
int MPILinearSearch(int *chunk, int size_array, int key, int *comparisons)
{
    // Starting traversal
    for (int i = 0; i < size_array; i++)
    {
        ...
        // Checking condition whether array value equal to key value
        if (chunk[i] == key)
        {
            // Element found, return the local index
            return i;
        }
    }
...
{
    // Initialize the array, size, key, and comparisons count variable
    int arr[] = {402,279,570,263,1,327,368,106,979,43,755,197,316,935,880,609,638,136,304,874,
    ...
    int numProcs, myRank, localIndex, globalIndex; 
    int key = 119; // Target value to search
    int comparisons = 0; // Variable to store the comparisons count
    ...
    // Calculate the size of the chunk that each process will handle
    int globalSize = sizeof(arr) / sizeof(arr[0]); // Size of the array
    int size_array = globalSize / numProcs;

    // Create an array to store the chunk of data from the arr[]
    int *chunk = (int *)malloc(size_array * sizeof(int));

    if(myRank == 0){
    
        // Scatter the chunk into each process
        MPI_Scatter(arr, size_array, MPI_INT, chunk, size_array, MPI_INT, 0, MPI_COMM_WORLD);

    }else{
        
        // Receive the chunk in each non-master process
        MPI_Scatter(arr, size_array, MPI_INT, chunk, size_array, MPI_INT, 0, MPI_COMM_WORLD);
    }

    // Calculate the time taken by each process
    ...
    if (localIndex != -1)
    {
        ...
        globalIndex = myRank * size_array + localIndex;
        printf("Process %d found the key value at its local index: %d and the global index: %d\n", myRank, localIndex, globalIndex);
        printf("\tThe element is present at index arr[%d]\n", globalIndex);
    }
    else
    {
        printf("Process %d did not find the key value\n", myRank); 
    }

    // Display the comparisons count
    printf("\tTotal comparisons made: %d\n", comparisons);

    // Display execution time for each process
    printf("\tExecution time for Process %d: %f seconds\n\n", myRank, executionTime);
